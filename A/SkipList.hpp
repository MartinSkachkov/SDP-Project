...
struct SkipListEl {
	...
	SkipListEl(const int& key, const T& town, SkipListEl<T>* jump = nullptr, SkipListEl<T>* next = nullptr) : mKey(key), mTown(town), mJump(jump), mNext(next) {}
};
...
public:
	...
	SkipListEl<T>* getHead()const;
	SkipListEl<T>* getTail()const;
	size_t getSize() const;
	size_t getKey(const T& town) const;
	SkipListEl<T>* getElem(const T& town) const;
...
template<typename T>
SkipListEl<T>* SkipList<T>::getHead()const {
	return mHead;
}

template<typename T>
SkipListEl<T>* SkipList<T>::getTail()const {
	return mTail;
}
...
template<typename T>
SkipListEl<T>* SkipList<T>::getElem(const T& town) const {
	if (empty()) {
...
void SkipList<T>::print() const {
	if (empty()) {
		throw "Can't traverse the empty list!";
	}

	SkipListEl<T>* slow = mHead;
	SkipListEl<T>* fast = mHead;
